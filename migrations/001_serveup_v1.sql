...
begin
  ...
  select * into v_row from public.v1_quiz_attempts where id = p_attempt_id;
  if found then
  ...
  if exists(select 1 from unnest(p_question_ids) id group by id having count(*) > 1) then raise exception 'Duplicate question'; end if;
  if (select count(*) from unnest(p_question_ids) id join public.v1_question_keys k on k.id = id and k.course_id = p_course_id) <> v_count then raise exception 'Unknown question'; end if;
  select count(*) into v_correct from unnest(p_question_ids) id join public.v1_question_keys k on k.id = id
    where p_answers ? id and (p_answers ->> id) ~ '^[0-3]$' and (p_answers ->> id)::integer = k.correct_answer;
